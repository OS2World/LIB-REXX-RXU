...
int qprintf(PCH qname, const unsigned char *f)
  ...
  /* If queue name supplied as argument, then use that.  Otherwise */
  /* use default name (PRINTFQNAME)                                */
  if (qname == NULL)
    ...
    urc=DosOpenQueue(&servepid, &qhandle, qworkarea  );  /* Open the Q */
    free(qworkarea);
    }
